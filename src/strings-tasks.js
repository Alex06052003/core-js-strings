...
function countVowels(str) {
  ...
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (arr.includes(str[i].toLowerCase())) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  const smallStr = str.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
  return smallStr.split('').reverse().join('') === smallStr;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (wordLength.length < arr[i].length) wordLength = arr[i];
  }
  return wordLength;
}
...
function invertCase(str) {
  ...
  let newStr = '';
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].charCodeAt(0) < 92) newStr += arr[i].toLowerCase();
    else newStr += arr[i].toUpperCase();
  }
  return newStr;
}
...
function encodeToRot13(str) {
  const arr = str.split('').map((item) => {
    ...
    if ((char > 64 && char < 78) || (char < 110 && char > 90))
      return String.fromCharCode(char + 13);
    if (char < 65 || char > 122 || (char > 90 && char < 97)) return item;
    return String.fromCharCode(char - 13);
  });
  return arr.join('');
}
